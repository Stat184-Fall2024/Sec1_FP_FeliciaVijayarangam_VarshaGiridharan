...
4.  What are the Air Travel Routes that are most impacted by the departing delays at the Philadelphia Airport?
...
# Research Question 4: What are the Air Travel Routes that are most impacted by the Departure delays at the Philadelphia Airport?
...
# Load airport data with IATA codes, latitude, and longitude
airports <- read.csv("C:/Users/felic/Downloads/us-airports.csv")

airports_coords <- airports %>%
  select(latitude_deg, iata_code, latitude_deg, longitude_deg)
...
#| message: False
#| warning: False
# Merge latitude/longitude with original data for ORIGIN
...
#| message: False
#| warning: False
# Merge latitude and longitude for DESTINATION
...
phl_departures <- flight_with_coords %>%
  filter(ORIGIN == "PHL", DEP_DELAY_NEW >0) %>%
  group_by(DEST)  %>%
  mutate(
...
#| message: False
#| warning: False
# Filter data to keep only top 5 destinations
...
ggplot() + 
  ...
  geom_point(
    data = phl_top_10, 
    aes(x = destLong, y = destLat), 
    col = "#970027", size = 2
  ) +  # Add destination points
  geom_text_repel(
    data = phl_top_10, 
    aes(x = destLong, y = destLat, label = DEST), 
    col = "black", size = 3, segment.color = "gray70"
