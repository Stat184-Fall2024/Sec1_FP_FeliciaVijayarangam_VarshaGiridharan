...
#REASEARCH Qs to think about:
#Passenger Perspective: What factors affect delays or cancellations? How do these vary across airlines or airports?
#Airline Perspective: Which routes have the highest delays? Are delays more common during specific times of the year?
#Regulatory Perspective: How effective are airlines in managing diverted flights? What are the common causes of cancellations?
...
flights2023 <- read_csv("~/Downloads/STAT184_FinalProject_Datasets/bts_dataset_2023.csv")

# Filter to view data between 20th and 27th December
christmas_week_2023_data <- flights2023 %>%
...
# clean data frame
cleandf_2023_data <- na.omit(christmas_week_2023_data)
...
ChristmasWeekFlights2023 <- cleandf_2023_data %>%
...
output_path <- "~/Downloads/STAT184_FinalProject_Datasets/ChristmasWeekFlights2023.csv"
write_csv(ChristmasWeekFlights2023, output_path)

# EDA
head(ChristmasWeekFlights2023)
summary(ChristmasWeekFlights2023)

# We find that there were no cancellations (CANCELLED) or flight diversions (DIVERTED) for the week of Christmas in 2023

#######                                                    [EDA]                                                     #######

# Use esquisser to find explore and find meaningful trends in data
#esquisser(data = ChristmasWeekFlights2023, viewer = "browser")
...
flights_by_airport <- ChristmasWeekFlights2023 %>%
  group_by(ORIGIN) %>%
  summarise(
...
top10_flights_by_airport <- flights_by_airport %>%
  ...
  left_join(ChristmasWeekFlights2023 %>% select(ORIGIN, Airport_Name), 
            by = "ORIGIN") %>%
  distinct()
...
flights_by_airline <- ChristmasWeekFlights2023 %>%
...
top10_flights_by_airline <- flights_by_airline %>%
  slice_max(order_by = Number_of_Flights, n = 10) %>%
  left_join(ChristmasWeekFlights2023 %>% select(OP_UNIQUE_CARRIER, Airline_Name), 
...
ggplot(top10_flights_by_airline, aes(x = reorder(Airline_Name, -Avg_Weather_Delay), y = Avg_Weather_Delay)) +
  ...
  labs(
    title = "Average Weather Delay by Top 10 Airlines (Christmas Week 2023)",
    x = "Airlines",
...
ggplot(top10_flights_by_airline, aes(x = reorder(Airline_Name, -Avg_Carrier_Delay), y = Avg_Carrier_Delay)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  labs(
    title = "Average Carrier Delay by Top 10 Airlines (Christmas Week 2023)",
    x = "Airlines",
...
ggplot(top10_flights_by_airport, aes(x = reorder(ORIGIN, -Avg_Weather_Delay), y = Avg_Weather_Delay)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  labs(
    title = "Average Weather Delay by Top 10 Airports (Christmas Week 2023)",
    x = "Airports",
    y = "Average Weather Delay (mins)"
  ) +
  theme_minimal() +
...
ggplot(top10_flights_by_airport, aes(x = reorder(ORIGIN, -Avg_Carrier_Delay), y = Avg_Carrier_Delay)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  labs(
    title = "Average Carrier Delay by Top 10 Airports (Christmas Week 2023)",
    x = "Airports",
    y = "Average Carrier Delay (mins)"
  ) +
  theme_minimal() +
...
# How do departure delays vary across the top 10 airlines during the Christmas week of 2023, and which airlines experience the most significant delays?"
top10_airlines <- top10_flights_by_airline$OP_UNIQUE_CARRIER

ChristmasWeekFlights2023 %>%
...
# How do arrival delays vary across the top 10 airlines during the Christmas week of 2023, and which airlines experience the most significant delays?
top10_airlines <- top10_flights_by_airline$OP_UNIQUE_CARRIER
...
# How do departure delays vary across the top 10 airports during the Christmas week of 2023, and which airports experience the most significant delays?
...
# How do arrival delays vary across the top 10 airports during the Christmas week of 2023, and which airports experience the most significant delays?
# Remove rows with non-finite ARR_DELAY values (N/A, <= 0)
filtered_data_arr <- ChristmasWeekFlights2023 %>%
...
ggplot(filtered_data_arr, aes(x = ORIGIN, y = ARR_DELAY)) + 
  ...
  theme_minimal() +
  labs(
    title = "Arrival Delay by Top 10 Airports During Christmas Week 2023",
    x = "Airport",
    y = "Arrival Delay (minutes, log scale)"
  ) +
  theme(
...
# How do weather-related delays vary across the top 10 airlines during Christmas week in 2023?
ChristmasWeekFlights2023 %>%
  filter(OP_UNIQUE_CARRIER %in% top10_airlines) %>%
...
# Which airlines among the top 10 experienced the highest carrier delays during Christmas week in 2023, and what might contribute to these delays?
ChristmasWeekFlights2023 %>%
  filter(OP_UNIQUE_CARRIER %in% top10_airlines) %>%
...
# How do weather-related delays at the top 10 airports compare during Christmas week in 2023, and which airports are most affected by adverse weather conditions?
ChristmasWeekFlights2023 %>%
  filter(ORIGIN %in% top10_airports) %>%
...
# How does the average departure delay change from the 20th to the 27th of December 2023, and are there any noticeable trends or patterns in the delays during this period?
ChristmasWeekFlights2023 %>%
...
# What trends are observed in the average arrival delay by day of the month from December 20th to 27th, 2023, and how do these trends relate to the overall travel demand during the holiday season?
ChristmasWeekFlights2023 %>%
  group_by(DAY_OF_MONTH) %>%
...
# How does the distribution of median delays (departure, arrival, carrier, and weather) vary across different days of the month (20th-27th) for the top 10 busiest airports during the Christmas week of 2023?"
...
# Do departure delays have a cascading effect? How do average departure delays correlate across the top 10 busiest airports during Christmas week in 2023, and are there patterns of delay propagation between airports in the same network or region?
...
top10_data <- ChristmasWeekFlights2023 %>%
  filter(ORIGIN %in% top10_airports) %>%
